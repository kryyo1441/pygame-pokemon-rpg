...
class Battle:
                ...
                if self.selection_mode == 'target':
                    ...
                    if self.selected_attack:
                        ...
                        if monster_sprite.monster.health < monster_sprite.monster.get_stat('max_health') * 0.9:
                            ...
                            TimedSprite(monster_sprite.rect.center, self.monster_frame['ui']['cross'], self.battle_sprites, 1000)
        ...
        for monster_sprite in self.opponent_sprites.sprites() + self.player_sprites.sprites():
            if monster_sprite.monster.health <= 0:
                if self.player_sprites in monster_sprite.groups():  # player
                    ...
                    new_monster_data = (list(self.monster_data['opponent'].values())[0],
                                        monster_sprite.index, monster_sprite.pos_index, 'opponent') if self.monster_data['opponent'] else None
    ...
    def draw_general(self):
        for index, (option, data_dict) in enumerate(BATTLE_CHOICES['full'].items()):
            if index == self.indexes['general']:
                ...
                surf = pygame.transform.grayscale(self.monster_frame['ui'][data_dict['icon']])
    ...
    def draw_switch(self):
        ...
        active_monsters = [(ms.index, ms.monster) for ms in self.player_sprites]
        ...
        for index, monster in enumerate(self.available_monsters.values()):
            ...
            icon_surf = self.monster_frame['icons'][monster.name]
            icon_rect = icon_surf.get_frect(midleft=bg_rect.topleft + vector(10, item_height / 2 + index * item_height + v_offset))
            text_surf = self.fonts['regular'].render(f'{monster.name} ({monster.level})',
                                                     False, COLORS['red'] if selected else COLORS['black'])
    ...
    def update(self, dt):
        ...
        self.battle_sprites.draw(self.current_monster, self.selection_side, self.selection_mode,
